...
public class AVProWindowsMedia : System.IDisposable
{
	...
	public bool StartVideo(string filename, bool allowNativeFormat, bool useBT709, bool useAudioMixer)
	{
		Filename = filename;
		if (!string.IsNullOrEmpty(Filename))
		{
			...
			if (AVProWindowsMediaPlugin.LoadMovie(_movieHandle, filenamePtr, false, allowNativeFormat, useAudioMixer))
			{
	...
	public bool StartVideoFromMemory(string name, System.IntPtr moviePointer, uint movieLength, bool allowNativeFormat, bool useBT709, bool useAudioMixer)
	{
		Filename = name;
		if (moviePointer != System.IntPtr.Zero && movieLength > 0)
		{
			...
			if (AVProWindowsMediaPlugin.LoadMovieFromMemory(_movieHandle, moviePointer, movieLength, allowNativeFormat, useAudioMixer))
			{
				CompleteVideoLoad(useBT709);
			}
			else
	...
	private void CompleteVideoLoad(bool useBT709)
	{
		...
		bool hasVideo = (Width > 0 && Width <= 4096 || Height > 0 && Height <= 4096);
		if (!hasVideo)
		{
			Width = Height = 0;
	...
	public bool StartAudio(string filename)
	{
		...
		if (!string.IsNullOrEmpty(Filename))
		{
			...
			// Note: we're marshaling the string to IntPtr as the pointer of type wchar_t 
			System.IntPtr filenamePtr = Marshal.StringToHGlobalUni(Filename);
			if (AVProWindowsMediaPlugin.LoadMovie(_movieHandle, filenamePtr, false, false, false))
			{
				Volume = _volume;
				DurationSeconds = AVProWindowsMediaPlugin.GetDurationSeconds(_movieHandle);
