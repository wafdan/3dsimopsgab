...
public class AVProWindowsMediaMovie : MonoBehaviour
{
	...
	public bool LoadMovie(bool autoPlay)
	{
		...
		if (_moviePlayer.StartVideo(filePath, allowNativeFormat, _colourFormat == ColourFormat.YCbCr_HD, _useAudioMixer))
		{
	...
	public bool LoadMovieFromMemory(bool autoPlay, string name, System.IntPtr moviePointer, uint movieLength)
	{
		...
		if (_moviePlayer.StartVideoFromMemory(name, moviePointer, movieLength, allowNativeFormat, _colourFormat == ColourFormat.YCbCr_HD, _useAudioMixer))
		{
			_moviePlayer.Volume = _volume;
			if (autoPlay)
			{
